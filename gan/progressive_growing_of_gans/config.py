...
# Dataset (choose one).
desc += '-p3_512x512_0_v3';            dataset = EasyDict(tfrecord_dir='p3_512x512_0_v3', resolution=512); train.mirror_augment = True
...
train = EasyDict(func='util_scripts.generate_interpolation_video', run_id=9, grid_size=[1,1], duration_sec=60.0, smoothing_sec=1.0); num_gpus = 1; desc = 'interpolation-video-' + str(train.run_id)
train = EasyDict(func='util_scripts.generate_training_video', run_id=9, duration_sec=20.0); num_gpus = 1; desc = 'training-video-' + str(train.run_id)
...
# train = EasyDict(func='util_scripts.evaluate_metrics', run_id=9, log='metric-fid-50k.txt', metrics=['fid'], num_images=50000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
